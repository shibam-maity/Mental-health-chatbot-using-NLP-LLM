...
class AgentCoordinator:
    ...
    def update_conversation_state(self, user_input, emotion, emotion_scores):
        ...
        if any(keyword in user_input.lower() for keyword in crisis_keywords):  # Fixed typo: Crisis_keywords -> crisis_keywords
...
class EmotionalSupportAgent:
    def __init__(self, sentiment_pipeline, groq_client, model_name):
        self.sentiment_pipeline = sentiment_pipeline
        self.groq_client = groq_client
        self.model_name = model_name

    def analyze_emotional_state(self, text: str) -> Tuple[str, Dict]:
        return analyze_sentiment(text)

    def generate_coping_strategies(self, emotion: str, intensity: float, topics: List[str] = None) -> str:
        topic_context = f"The user has discussed these topics: {', '.join(topics)}." if topics else ""

        messages = [
            {"role": "system", "content": "You are an AI specialized in providing emotional support and coping strategies. Be concise and practical."},
            {"role": "user", "content": f"Suggest 2-3 practical coping strategies for someone experiencing {emotion} with intensity {intensity:.2f}. {topic_context} Keep your response under 150 words and focus on actionable techniques."}
        ]
        ...
        try:
            ...
            return f"Sorry, I encountered an issue generating coping strategies. Error: {str(e)}"
...
emotional_support = EmotionalSupportAgent(sentiment_pipeline, groq_client, MODEL_NAME)
